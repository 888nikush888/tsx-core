...
type EvidenceAmounts = {
  status?: string | null;
  additionalRiskValue?: unknown;
  additionalRisk?: unknown;
...
type EvidenceRow = Record<string, unknown> & {
  ...
  fxId?: string | null;
  amounts?: EvidenceAmounts | null;
};
type EvidenceObservationRef = { id: string; observedAt?: unknown; timestampFresh?: boolean | null; isCurrentObservation?: boolean | null };
type EvidenceAccount = { name?: string | null; exchange?: string | null; mode?: string | null };
type DailyEvidence = {
  since: number;
  until: number;
  reportingCurrency?: string | null;
  valuationStatus?: string | null;
  pricePnlValue?: unknown;
  ...
  unresolvedEventCount?: number | null;
  conflictCount?: number | null;
  pendingProjections?: number | null;
  unresolvedProjections?: number | null;
  valuationHash?: string | null;
};
type RiskEvidence = {
  id: string;
  observedAt?: unknown;
  ...
  credentialGenerationMatches?: boolean | null;
  reservationCount?: number | null;
  balanceSourceObservationId?: string | null;
  balanceObservedAt?: unknown;
...
type EvidenceFailure = { source?: string | null; reason?: string | null };
type EvidenceBaseline = { id?: string | null; boundary?: unknown };
type EvidencePayload = {
  entries: EvidenceRow[];
  interpretation?: string;
  hasMore?: boolean;
  nextCursor?: string | null;
  observation?: EvidenceObservationRef | null;
  account: EvidenceAccount;
  daily: DailyEvidence;
  risk: RiskEvidence | null;
  errors?: EvidenceFailure[] | null;
  observedAt?: unknown;
  baseline?: EvidenceBaseline | null;
  requiredHistorySince?: unknown;
};
type EvidenceObservation = { url: string; value: EvidencePayload };

function useEvidence(url: string) {
  const [state, setState] = useState<EvidenceObservation | null>(null); const [error, setError] = useState('');
...
export function RiskAccounts() {
  ...
  return <section className="space-y-4"><h1>Risiko- und Historienbelege je Konto</h1><p>Kontomodell und Währung bleiben getrennt. Ein lesbarer oder frischer Beleg ist keine neue Entry-Freigabe.</p>
    {error && <p role="alert">{error}</p>}{data ? <><EvidenceTable caption="Konten" columns={[["name", "Kontobelege öffnen"], ["exchange", "Börse"], ["mode", "Modus"], ["status", "Status"], ["retiredAt", "Entfernt"]]} rows={data.entries.map(row => ({ ...row, name: <Link to={`/risk/accounts/${encodeURIComponent(row.id as string)}`}>{row.name}</Link>, retiredAt: time(row.retiredAt) }))} />
...
export function RiskAccountEvidence({ accountId }: Readonly<{ accountId: string }>) {
  ...
  const money = (value: unknown, amount: string | null) => <MoneyAmount value={value} amount={amount} currency={daily.reportingCurrency} status={daily.valuationStatus === 'valued' ? 'complete' : 'unresolved'} />;
  return <section className="space-y-5"><h1>{account.name} · Risiko & Historie</h1><p>{account.exchange}/{account.mode} · gelesen {time(data.observedAt)}.</p>
    ...
    {data.errors?.map(failure => <p key={failure.source} role="alert">{failure.source}: {failure.reason}</p>)}<section className="operations-card space-y-3"><h2>Abrechnung im UTC-Risikotag</h2>{daily ? <><p>{new Date(daily.since).toISOString()} bis {new Date(daily.until).toISOString()} · Anzeigezeit lokal: {time(daily.until)}.</p>
