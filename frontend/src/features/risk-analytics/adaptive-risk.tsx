...
function SourceEvidence({ id, channelId }: { id: string; channelId?: string }) {
...
function ActivePolicyPaths({ stateKey }: { stateKey: string }) {
...
function EvaluationCard({ row, legacyChannel }: { row: any; legacyChannel?: string }) {
...
function LegacyCard({ entry }: { entry: any }) {
  ...
  const copy = async () => {
    ...
    catch (failure) { setError(`Kopie nicht bestätigt: ${failure instanceof Error ? failure.message : String(failure)}. Vor einer weiteren Aktion den gespeicherten Entwurf prüfen.`); }
  ...
  return <article className="operations-card space-y-3">{confirmationDialog}<h2>Legacy · Kanal {policy.channelId}</h2>
    ...
    {receipt && <p role="status">Entwurf {receipt.alreadyCopied ? 'bereits vorhanden' : 'gespeichert'}; nicht aktiviert. <Link to={resourceUrl(receipt.resource)}>Ressourcenentwurf öffnen</Link></p>}
...
export function AdaptiveRiskPage() {
  ...
  return <section className="space-y-5"><h1>Adaptive Risikopolicen</h1>
    ...
    {data ? <><p>{data.interpretation} Gelesen {time(data.observedAt)}.</p>
      {data.entries.map((row: any) => kind === 'legacy' ? <LegacyCard key={row.policy.channelId} entry={row} /> : kind.includes('evaluations') ? <EvaluationCard key={row.id} row={row} legacyChannel={kind === 'legacy-evaluations' ? params.get('channelId')! : undefined} />
        : <article key={row.stateKey} className="operations-card space-y-4"><h2>{row.accountName} · {row.mode} · {row.channelId}</h2>
      ...
      : !error && <p role="status">Risikopolicen werden geladen …</p>}
  </section>;
}
