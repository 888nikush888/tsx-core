...
export function TelegramSettings() {
  ...
  const command = async (url: string, body: unknown, accepted: (result: any) => void, description: string, headers: Record<string, string> = {}) => {
    ...
    try {
      await mutateAndObserve(() => jsonRequest(url, { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify(body) }), result => { setMessage(`${description}${result.requestId ? ` · Anfrage ${result.requestId}` : ''}`); accepted(result); }, async () => setRefresh(value => value + 1));
  ...
  return <div className="operations-stack"><h2>Telegram & KI-Grundlage</h2>
    ...
    {message && <p role="status">{message}</p>}{readOnly && <p>Nur Lesezugriff. Änderungen und Anmeldung benötigen die Adminrolle.</p>}
    ...
    {!readOnly && <section className="operations-card system-form"><h3>Telegram-/KI-Zugangsdaten</h3><fieldset disabled={busy || !secrets}>{[['telegramApiHash', 'Telegram API Hash'], ['openRouterApiKey', 'OpenRouter API Key']].map(([key, label]) => <label key={key}>{label} · {secrets?.[key]?.configured ? 'gespeichert' : secrets ? 'fehlt' : 'unbekannt'}<input type="password" autoComplete="off" placeholder="Leer lassen zum Beibehalten" value={secretInput[key] ?? ''} onChange={event => setSecretInput({ ...secretInput, [key]: event.target.value })} /></label>)}
