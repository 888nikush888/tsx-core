...
type SignalEntry = Record<string, unknown> & {
  ...
  confirmedMessageIds?: string | number | null;
  reason?: string | null;
...
type SignalsObservation = { query: string; kind: string; payload: SignalsPayload };
type OutboxTask = { id: string; targetChatId?: string | null; status?: string | null };
type IngressDetailPayload = {
  work: { channelId?: string | null; messageId?: string | number | null; workflowRevisionId?: string | null; status?: string | null; reason?: string | null };
...
export function SignalsPage({ kind, readOnly = true }: Readonly<{ kind: "ingress" | "processed" | "outbox" | "messages"; readOnly?: boolean }>) {
  ...
  const [page, setPage] = useState<SignalsObservation | null>(null);
  ...
  const command = async (task: OutboxTask, action: "retry" | "acknowledge") => {
    if (readOnly) return;
  ...
  const signalPageTitle = () => {
    if (kind === "outbox") {
      return "Versand & Outbox";
    }
    if (kind === "ingress") {
      return "Dauerhafter Eingang & Alben";
    }
    if (kind === 'messages') {
      return 'Gespeicherte Nachrichten';
    }
    return "Parserergebnisse";
  };
...
export function IngressDetail({ id }: Readonly<{ id: string }>) {
  const [detail, setDetail] = useState<IngressDetailPayload | null>(null);
  const [error, setError] = useState("");
