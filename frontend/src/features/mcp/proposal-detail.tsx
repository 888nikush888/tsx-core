...
export function ProposalDetail({ id }: { id: string }) {
  ...
  const decide = async (approve: boolean) => {
    ...
    } catch (failure) { setMessage(`Entscheidung nicht bestätigt: ${failure instanceof Error ? failure.message : String(failure)}. Status prüfen; keine automatische Wiederholung.`); }
    finally { setBusy(false); }
  };
  if (!review) return <section><h1>MCP-Vorschlag</h1><p role={error ? 'alert' : 'status'}>{error || 'Prüfinhalt wird geladen …'}</p></section>;
  ...
  return <section className="space-y-5"><Link to="/integrations/mcp">MCP & Agenten</Link><h1>Vorschlag prüfen · {proposal.action}</h1>
    {error && <p role="alert">{error} · Angezeigte Vorschau kann veraltet sein.</p>}{message && <p role="status">{message}</p>}
    ...
    <section><h2>Voraussetzungen</h2><p>Ursprünglicher Preflight: {time(proposal.preflight?.checkedAt)} · Alter {Math.max(0, Math.floor((review.observedAt - proposal.preflight?.checkedAt) / 1000))} s</p>
      ...
      <ul>{[...(review.freshPreflight.blockers ?? []), ...(review.freshPreflight.impact ?? [])].map((item: string, index: number) => <li key={index}>{item}</li>)}</ul></section>
