...
type ReviewEntry = { key: string; type: string; value: unknown; childCount?: number | null; expandable: boolean; path?: string[] };
type ReviewPage = { type: string; text?: string; entries: ReviewEntry[]; value?: unknown; hasMore: boolean; nextCursor: string | null };
type ReviewObservation = { query: string; value: ReviewPage };
...
export function SetupReviewTree({ previewKey }: Readonly<{ previewKey: string }>) {
  ...
  const [state, setState] = useState<ReviewObservation | null>(null); const [error, setError] = useState('');
  ...
  return <section className="space-y-4" aria-label="Vollständiger Setup-Prüfbestand"><h3>Vollständige Inhalte abschnittsweise prüfen</h3>
    ...
    {data ? <>{data.type === 'string' ? <pre className="whitespace-pre-wrap break-all border p-3 max-h-96 overflow-auto">{data.text}</pre> : <EvidenceTable caption="Originalwerte dieser Ebene" columns={[["key", "Feld / Objekt"], ["type", "Datentyp"], ["value", "Gespeicherter Wert / Anfang"], ["childCount", "Unterwerte"], ["open", "Inhalt öffnen"]]} rows={data.entries.map((entry) => ({ ...entry, value: entry.type === 'null' ? 'null' : entry.value,
      open: entry.expandable && entry.path ? <button className="secondary-button" onClick={() => { if (entry.path) open(entry.path); }}>{entry.key} vollständig öffnen</button> : 'vollständig angezeigt' }))} />}
      {!data.entries.length && data.type !== 'string' && <p>Wert: {JSON.stringify(data.value ?? null)}</p>}
      <div className="flex gap-3"><button className="secondary-button" disabled={!cursor} onClick={() => setCursor('')}>Erster Prüfabschnitt</button><button className="secondary-button" disabled={!data.hasMore} onClick={() => { if (data.nextCursor) setCursor(data.nextCursor); }}>Weitere Prüfwerte</button></div>
