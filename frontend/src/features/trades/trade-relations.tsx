...
// ui_trade_relations returns redacted rows; money fallback omits verified evidence.
// Preserve unknown scalar evidence for MoneyAmount and EvidenceTable validation.
type RelationEntry = Record<string, unknown> & {
  id: string; originalUnverified?: boolean; explanation?: string; valuationReason?: string | null; eventType?: string;
};
type RelationPage = {
  entries: RelationEntry[]; observedAt: number; hasMore: boolean; nextCursor: string | null;
};
...
export function TradeRelations({ intentId, kind }: Readonly<{ intentId: string; kind: Kind }>) {
  ...
  const [state, setState] = useState<{ query: string; page: RelationPage } | null>(null); const [error, setError] = useState('');
  ...
  const rows = (page?.entries ?? []).map((row) => ({ ...row, ...(row.filledAt !== undefined ? { filledAt: time(row.filledAt) } : {}), ...(row.occurredAt !== undefined ? { occurredAt: time(row.occurredAt) } : {}),
