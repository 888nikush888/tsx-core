...
export function TradingList({ kind }: Readonly<{ kind: TradingListKind }>) {
  ...
  const statusFilter = (<label>Status<select value={params.get('status') ?? ''} onChange={(event) => change('status', event.target.value)}><option value="">Alle</option>{(page?.states ?? []).map((status: string) => <option key={status}>{status}</option>)}</select></label>);
  return <div className="operations-stack"><h1>{LISTS[kind].title}</h1>
    <div className="operations-card system-form flex flex-wrap gap-4"><label>Konto-ID<input value={params.get('accountId') ?? ''} onChange={(event) => change('accountId', event.target.value)} maxLength={128} /></label>
      ...
      {statusFilter}<button className="secondary-button" onClick={() => setParams(new URLSearchParams())}>Filter zurücksetzen</button></div>
