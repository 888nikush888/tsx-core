...
export function ModelLibrary({ kind, id }: { kind: Kind; id?: string }) {
  ...
  const command = async (action: string) => {
    ...
    } catch (failure) { setError(`Aktion nicht bestätigt: ${failure instanceof Error ? failure.message : String(failure)}. Keine automatische Wiederholung.`); }
  ...
  return <section className="space-y-5">{confirmationDialog}<h1>Modellbibliothek · {titles[kind]}</h1>
    ...
    {receipt && <div role="status"><p>Modellaktion {receipt.action} bestätigt. Keine Graphaktivierung oder Handelsausführung.</p>{receipt.resource && <Link to={resourceUrl(receipt.resource)}>Gespeicherten Ressourcenentwurf öffnen</Link>}</div>}
    {!data ? !error && <p role="status">Modelle werden geladen …</p> : id ? <>
      ...
      <div className="flex flex-wrap gap-3"><button className="primary-button" disabled={readOnly || busy || data.model.status === 'archived'} onClick={() => void command('attach')}>Als Ressourcenentwurf übernehmen</button>
        {kind !== 'schema' && data.model.status === 'draft' && <button className="secondary-button" disabled={readOnly || busy} onClick={() => void command('publish')}>Modell publizieren</button>}
        {kind !== 'schema' && data.model.status === 'published' && <button className="secondary-button" disabled={readOnly || busy || data.activeReferenceCount > 0} onClick={() => void command('archive')}>Modell archivieren</button>}
        {kind === 'schema' && <button className="secondary-button" disabled={readOnly || busy || data.activeReferenceCount > 0} onClick={() => void command(data.model.enabled ? 'disable' : 'enable')}>{data.model.enabled ? 'Profil deaktivieren' : 'Profil aktivieren'}</button>}
        <button className="secondary-button" disabled={readOnly || busy || data.resourceCount > 0} onClick={() => void command('delete')}>Modell löschen</button></div>
    ...
    </> : <><p>Beobachtet {time(data.observedAt)} · {data.hasMore ? 'Weitere Seiten vorhanden' : 'Ende der Auswahl'}</p>
