...
export function WorkflowLibrary({ kind, resourceId }: { kind: Kind; resourceId?: string }) {
  ...
  const rows = (page?.entries ?? []).map((entry: any) => ({ ...entry,
    id: <Link to={kind === 'resources' ? resourceUrl(entry) : `/workflows/${kind}/${encodeURIComponent(entry.id)}`}>{kind === 'resources' ? `${entry.name} · v${entry.version}` : kind === 'revisions' ? `Revision ${entry.revision}` : entry.id}</Link>,
  ...
  const columns: Array<[string, string]> = kind === 'resources' ? [['kind', 'Bausteinart'], ['status', 'Zustand'], ['editRevision', 'Entwurfsstand']] : kind === 'paths' ? [['channelId', 'Kanal'], ['accountId', 'Konto'], ['fallbackRank', 'Fallbackrang (0 = primär)'], ['enabled', 'In dieser Revision aktiviert']] : [['status', 'Zustand'], ['createdBy', 'Erstellt von']];
...
export function WorkflowObject({ kind, id, resourceId }: { kind: Kind; id: string; resourceId?: string }) {
  ...
  const command = async (operation: () => Promise<any>, label: string, accept?: (value: any) => void) => {
    ...
    } catch (failure) { setError(`Aktion nicht bestätigt: ${failure instanceof Error ? failure.message : String(failure)}. Keine automatische Wiederholung.`); return null; }
  ...
  const lifecycle = async (operation: 'publish' | 'archive' | 'delete', family = false) => {
    ...
    if (!await confirm({ title: operation === 'publish' ? 'Ressourcenversion publizieren' : removal ? 'Ressource dauerhaft löschen' : 'Ressource archivieren',
      description: `${resource.name} v${resource.version}${family ? ' · alle Versionen dieser Familie' : ''}. ${operation === 'publish' ? `Wird unveränderlich; referenziertes Modell ${data.publication?.dependency?.id ?? 'keines'} wird gegebenenfalls mitpubliziert. Die Aktivierung im Signalweg folgt separat.` : 'Der Server prüft alle aktiven und historischen Referenzen. Eine Referenzsperre kann hier nicht umgangen werden.'}`,
      confirmationText: removal ? 'RESSOURCE LÖSCHEN' : undefined, confirmLabel: operation === 'publish' ? 'Publizieren' : removal ? 'Löschen' : 'Archivieren', destructive: operation !== 'publish' })) return;
    await command(() => jsonRequest(operation === 'publish' ? '/api/workflow/resources/publish' : '/api/workflow/resources', { method: operation === 'publish' ? 'POST' : 'DELETE',
      headers: { 'X-Destructive-Confirmation': operation === 'publish' ? 'publish-workflow-dependencies' : operation === 'delete' ? 'delete-workflow-resource-permanently' : 'delete-workflow-resource' },
    ...
    operation === 'publish' ? 'Publikation bestätigt. Noch keine neue Graphaktivierung.' : removal ? 'Löschung bestätigt; die alte Objektadresse kann nun 404 melden.' : 'Archivierung bestätigt.', result => { if (result.resource) setData((current: any) => ({ ...current, resource: result.resource })); });
  };
  ...
  return <section className="space-y-5">{confirmationDialog}<Link to={`/workflows/${kind}`}>{TITLES[kind]}</Link><h1>{resource ? `${resource.name} · Version ${resource.version}` : kind === 'paths' ? 'Originaler Ausführungspfad' : `Workflowrevision ${data.revision.revision}`}</h1>
    ...
    {resource ? <><EvidenceFields fields={[["Ressource", resource.resourceId], ["Version-ID", resource.id], ["Zustand", resource.status], ["Entwurfsrevision", resource.editRevision], ["Konfigurationshash", resource.configurationSha256], ["Publiziert", time(resource.publishedAt)]]} />
      ...
      <button className="secondary-button" disabled={!comparisonId} onClick={() => void (async () => { try { const result = await jsonRequest(`/api/workflow/objects?kind=resources&id=${encodeURIComponent(comparisonId)}`); if (result.resource.resourceId !== resource.resourceId) throw new Error('Vergleich erfordert dieselbe Ressourcenfamilie.'); setComparison(result.resource); } catch (failure) { setError(String(failure)); } })()}>Versionen vergleichen</button>
      ...
      {data.path && <><EvidenceFields fields={[["Pfad-ID", data.path.id], ["Kanal", data.path.channelId], ["Konto", <Link key="account" to={`/trading/accounts/${encodeURIComponent(data.path.accountId)}`}>{data.path.accountId}</Link>], ["Fallbackrang", data.path.fallbackRank]]} />
        <EvidenceTable caption="Wirksame Strategieparameter und Ursprung" rows={(data.parameterEffects ?? []).map((field: any) => ({ ...field,
          value: field.value === null ? 'null' : typeof field.value === 'object' ? JSON.stringify(field.value) : field.value,
          strategyValue: !field.strategyValuePresent ? 'nicht gesetzt' : field.strategyValue === null ? 'null' : typeof field.strategyValue === 'object' ? JSON.stringify(field.strategyValue) : field.strategyValue,
      ...
      {data.revision.warnings.map((warning: string, index: number) => <p key={index}>{warning}</p>)}
