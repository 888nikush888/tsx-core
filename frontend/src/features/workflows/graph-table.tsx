...
export function GraphTable({ graph, resources, readOnly, edit, connect, remove }: { graph: WorkflowGraph; resources: WorkflowResource[]; readOnly: boolean;
  ...
  return <section className="operations-card system-form"><h2>Graph als Tabelle</h2><label>Bausteine filtern<input value={filter} onChange={event => setFilter(event.target.value)} /></label>
    ...
    <fieldset disabled={readOnly}><legend>Normale Verbindung anlegen</legend><label>Von<select value={source} onChange={event => setSource(event.target.value)}><option value="">Quelle wählen</option>{graph.nodes.map(node => <option key={node.id} value={node.id}>{name(node.id)} · {node.kind}</option>)}</select></label><label>Nach<select value={target} onChange={event => setTarget(event.target.value)}><option value="">Ziel wählen</option>{graph.nodes.filter(node => node.id !== source).map(node => <option key={node.id} value={node.id}>{name(node.id)} · {node.kind}</option>)}</select></label><button className="primary-button" disabled={!source || !target} onClick={() => connect(source, target)}>Verbindung im Entwurf anlegen</button></fieldset><p>Der Server prüft Reihenfolge und erlaubte Bausteinkombinationen. Fallbacks werden mit ihren sicheren Gründen und ihrem Kanalscope im bestehenden Fallbackformular bearbeitet.</p>
  </section>;
}
