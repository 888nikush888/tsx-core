...
function IncidentsAlert({ incidents, trading, onOpenIncidents }: Readonly<{
  incidents: TradingSnapshot['accountIncidents']; trading: TradingSnapshot | null; onOpenIncidents?: () => void;
}>) {
  ...
  return (
    <section className="operations-card critical-dashboard-alert" aria-live="assertive">
      <AlertTriangle />
      <div>
        <h3>{incidents.length} aktive Konto-Incident{incidents.length === 1 ? "" : "s"}</h3>
        {incidents.slice(0, 3).map((incident) => <p key={incident.id}>{accountById.get(incident.accountId)?.name || incident.accountId}: {incident.message} · {incident.occurrenceCount}×</p>)}
        {onOpenIncidents && (
...
function CockpitMetrics({ portfolioTotal, overview, trading, positions, pendingIntents, unknownOrders }: Readonly<{
  portfolioTotal: (key: string) => ReturnType<typeof portfolioSnapshotTotal>; overview: TradingSnapshot['overview'] | undefined;
  trading: TradingSnapshot | null; positions: Position[]; pendingIntents: Intent[]; unknownOrders: Order[];
}>) {
  const danger = (overview?.unknownOrderCount ?? 0) > 0 ? "danger" : "";
  return (
    <>
      ...
      <div className="operations-metrics">
        ...
        <div className={`operation-metric ${danger}`}>
          ...
          {trading && positions.length === 0 && <small style={{ color: "var(--muted-foreground)" }}>Keine Position</small>}
        </div>
        <div className="operation-metric">
          <strong>{overview?.pendingIntentCount ?? "unbekannt"}</strong><span>Wartende Intents</span>
          {pendingIntents.slice(0, 5).map((i) => (
            <small key={i.id} style={{ display: "block", marginTop: 4 }}>{i.symbol || i.channelId} · {i.status}</small>
          ))}
          {trading && pendingIntents.length === 0 && <small style={{ color: "var(--muted-foreground)" }}>Keine Intents</small>}
        </div>
        <div className={`operation-metric ${danger}`}>
          <strong>{overview?.unknownOrderCount ?? "unbekannt"}</strong><span>Unklare Orders</span>
          {unknownOrders.slice(0, 5).map((o) => (
            <small key={o.id} style={{ display: "block", marginTop: 4 }}>{o.symbol || o.intentId} · {o.status}</small>
          ))}
          {trading && unknownOrders.length === 0 && <small style={{ color: "var(--muted-foreground)" }}>Keine unklaren Orders</small>}
        </div>
      </div>
    </>
  );
...
function DashboardGrid({ trading, access, portfolio, overview }: Readonly<{
  trading: TradingSnapshot | null; access: Access | null; portfolio: Portfolio | null; overview: TradingSnapshot['overview'] | undefined;
}>) {
...
function OpenPositionsSection({ trading, positions }: Readonly<{ trading: TradingSnapshot | null; positions: Position[] }>) {
  ...
  return (
    <section className="operations-card">
      ...
      <div className="position-table" role="table" aria-label="Aktive Positionen">
        ...
        {trading && positions.length === 0 && <Empty text="Keine aktive Position." />}
      </div>
    </section>
  );
}

function SignalsIntentsGrid({ signals, trading }: Readonly<{ signals: Signal[] | null; trading: TradingSnapshot | null }>) {
  const accountById = new Map((trading?.accounts || []).map((account) => [account.id, account]));
  const openIntents = ((trading?.intents as Intent[] | undefined) || []).filter((intent) => ["pending", "planned", "submitting", "monitoring", "unknown"].includes(intent.status));
  return (
    <div className="dashboard-grid">
      <section className="operations-card">
      ...
      <section className="operations-card">
        ...
        {openIntents.slice(0, 5).map((intent) => <div className="adaptive-row" key={intent.id}><div><strong>{intent.symbol} · {intent.side}</strong><small>{intent.channelId} → {accountById.get(intent.accountId)?.name || intent.accountId}</small></div><span className={`state-badge ${intent.status === "unknown" ? "danger" : ""}`}>{intent.status}</span></div>)}
        {trading && openIntents.length === 0 && <Empty text="Keine offenen Intents." />}
      </section>
    </div>
  );
}

function FallbackRunsSection({ trading }: Readonly<{ trading: TradingSnapshot | null }>) {
  const fallbackRuns = trading?.fallbackRuns || [];
  return (
    <section className="operations-card">
      <h3>Letzte Börsen-Fallbacks</h3>
      {fallbackRuns.slice(0, 8).map((run) => {
...
function TradingControls({ disabled, runtime, onExecution, onLive, onKillSwitch, onReconcile, onCancel, onFlatten }: Readonly<{
  disabled: boolean; runtime: TradingSnapshot['overview']['runtime'] | undefined; onExecution: () => void; onLive: () => void;
  onKillSwitch: () => void; onReconcile: () => void; onCancel: () => void; onFlatten: () => void;
}>) {
  return (
    <section className="operations-card">
...
function RiskEventsSection({ trading }: Readonly<{ trading: TradingSnapshot | null }>) {
  return (
    <section className="operations-card">
...
export function Overview({
  ...
  const pendingIntents = ((trading?.intents as Intent[] | undefined) || []).filter((intent) => ["pending", "planned", "submitting"].includes(intent.status));
  const unknownOrders = ((trading?.activity.orders as Order[] | undefined) || []).filter((order) => order.status === "unknown");
  const openIncidents = (trading?.accountIncidents || []).filter((incident) => incident.status === "open");
  const controlsDisabled = Boolean(busy) || readOnly || !runtime;
  ...
  const emergencyFlatten = async () => {
   ...
   return (
    <div className="operations-stack">
      ...
      <IncidentsAlert incidents={openIncidents} trading={trading} onOpenIncidents={onOpenIncidents} />
      <CockpitMetrics portfolioTotal={portfolioTotal} overview={overview} trading={trading} positions={openPositions} pendingIntents={pendingIntents} unknownOrders={unknownOrders} />
      <section className="operations-card">
      ...
      <DashboardGrid trading={trading} portfolio={portfolio} access={access} overview={overview} />
      <OpenPositionsSection trading={trading} positions={openPositions} />
      <SignalsIntentsGrid signals={signals} trading={trading} />
      <FallbackRunsSection trading={trading} />
      <TradingControls disabled={controlsDisabled} runtime={runtime} onExecution={() => { mutate("execution", "/api/trading/runtime", { action: "execution", enabled: runtime?.executionEnabled !== true }); }} onLive={() => { setLive(); }} onKillSwitch={() => { setKillSwitch(); }} onReconcile={() => { mutate("reconcile", "/api/trading/reconcile", {}); }} onCancel={() => { mutate("cancel", "/api/trading/cancel-entries", {}); }} onFlatten={() => { emergencyFlatten(); }} />
      <RiskEventsSection trading={trading} />
    </div>
  );
}
