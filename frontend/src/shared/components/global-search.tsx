...
export function GlobalSearch() {
  ...
  const [kind, setKind] = useState('all'); const [cursor, setCursor] = useState(''); const [state, setState] = useState<SearchObservation | null>(null); const [error, setError] = useState('');
  ...
  const load = useCallback(async (signal: AbortSignal): Promise<SearchObservation> => ({ key, result: open && query.length >= 2
  ...
  const searchContent = () => {
    if (query.length < 2) {
      return <p>Mindestens zwei Zeichen eingeben.</p>;
    }
    if (!result && !error) {
      return <p><output>Suche läuft …</output></p>;
    }
    return result?.groups.map((group) => <section key={group.kind} aria-label={categories[group.kind]} className="space-y-2"><h3>{categories[group.kind]}</h3>
      {group.entries.length ? <ul className="space-y-2">{group.entries.map((item) => <li key={item.id} className="border-b pb-2 break-words"><Link className="underline" to={item.url} onClick={close}>{item.title}</Link><p className="text-sm">{item.subtitle ?? item.id}</p></li>)}</ul> : <p>Keine Treffer für diese Auswahl.</p>}
