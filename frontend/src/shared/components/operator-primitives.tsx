...
export function Metric({ label, value, danger = false }: Readonly<{ label: string; value: ReactNode; danger?: boolean }>) {
  return <div className={`operation-metric ${danger ? 'danger' : ''}`}><strong>{value}</strong><span>{label}</span></div>;
}
export function Empty({ text }: Readonly<{ text: string }>) { return <div className="operations-empty">{text}</div>; }
