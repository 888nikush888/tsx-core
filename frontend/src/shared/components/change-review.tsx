...
export function ChangeReview({ before, after, label = 'Inhaltliche Änderungen', showAll = false }: { before?: unknown; after: unknown; label?: string; showAll?: boolean }) {
  ...
  return <section aria-label={label} className="space-y-3"><h3>{label}</h3>
    ...
    <div className="overflow-x-auto" tabIndex={0} role="group" aria-label={`${label}: Tabelleninhalt`}><table className="w-full text-sm"><caption className="sr-only">{label}</caption><thead><tr><th scope="col">Feld</th>{before !== undefined && <th scope="col">Vorher</th>}<th scope="col">{before === undefined ? 'Inhalt' : 'Beantragt'}</th></tr></thead>
      <tbody>{rows.slice(currentPage * 40, (currentPage + 1) * 40).map(row => <tr key={row.path}><th scope="row" className="max-w-64 break-all border-b p-2 text-left">{row.path}</th>{before !== undefined && <td className="max-w-96 whitespace-pre-wrap break-all border-b p-2 align-top">{row.left}</td>}<td className="max-w-96 whitespace-pre-wrap break-all border-b p-2 align-top">{row.right}</td></tr>)}</tbody></table></div>
