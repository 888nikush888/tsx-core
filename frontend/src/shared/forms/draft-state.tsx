export function DraftState({ label, form, server }: Readonly<{ label: string; form: { dirty: boolean; conflict: boolean; draft: unknown; acceptServer: () => void; rebase: () => void }; server: unknown }>) {
  if (!form.dirty && !form.conflict) return null;
  return <section aria-label={`${label} Entwurf`} className="operations-card"><p><output>{label}: {form.dirty ? 'Ungespeicherte Änderungen' : 'Unveränderter Entwurf'}. {form.conflict && 'Serverstand geändert. Speichern ist bis zum Vergleich gesperrt.'}</output></p>
