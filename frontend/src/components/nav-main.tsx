          ...
          >
            <SidebarMenuItem>
              {item.items?.length ? (
                ...
                <SidebarMenuButton asChild tooltip={item.title} className="cursor-pointer" isActive={(location.pathname + location.search) === item.url || (location.pathname === item.url && !location.search)}>
                  <Link to={item.url}>
                    {item.icon && <item.icon />}
                    <span>{item.title}</span>
                  </Link>
