...
async function createLocalSession(): Promise<AccessInspection | null> {
  ...
  if (!response.ok || typeof payload.token !== "string") {
    ...
    throw new Error(
      payload.error || "Local dashboard access could not be initialized.",
    );
  }
...
async function inspectConfiguredAccess(
  ...
  return {
    state: "locked",
    error:
      "Für den Remote-Zugriff ist ein gültiger Bearer-Token oder eine konfigurierte Identität erforderlich.",
  };
}
...
async function inspectDashboardAccess(): Promise<AccessInspection> {
  ...
  const bootstrap: BootstrapInspection & { error?: string } =
    await bootstrapResponse.json().catch(() => ({}));
  if (!bootstrapResponse.ok)
    throw new Error(bootstrap.error || "Could not inspect dashboard setup.");
      ...
      <main className="min-h-screen grid place-items-center bg-background p-6">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Create the first administrator access token. It is generated by
              the server and stored in Docker's persistent secret volume.
            </CardDescription>
          </CardHeader>
      ...
      <main className="min-h-screen grid place-items-center bg-background p-6">
        <Card className="w-full max-w-xl">
          <CardHeader>
            ...
            <CardDescription>
              This token is shown once. Docker also keeps it in the persistent
              secret volume and this browser keeps it only for the current
              session.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={submit}>
              ...
              <Button
                className="w-full"
                disabled={submitting || token.trim().length < 32}
                type="submit"
              >
