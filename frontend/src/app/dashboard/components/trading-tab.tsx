...
const DEFAULT_CONFIGURATION = {
  ...
  sizing: { positionSizingMode: "risk_percent", riskPerTradePercent: "1", maxAdaptiveRiskPercent: "1", maxPositionNotional: "1000", maxLeverage: 3 },
  exits: {
  ...
  safety: { maxConcurrentPositions: 1, maxDailyLossMode: "absolute", maxDailyLoss: "100", maxSlippagePercent: "0.5", entryOrderTtlSeconds: 900, requireProtectiveStop: true },
}
...
const EMPTY_SIGNAL_SCHEMA = { id: "", name: "", description: "", contractVersionId: "", templateName: "", enabled: true }

function SignalSchemaManager({ data, busy, run }: any) {
  const initialContractVersionId = (Array.isArray(data.signalContracts) ? data.signalContracts : [])
    .flatMap((contract: any) => contract.versions)
    .find((version: any) => version.status === "published")?.id || ""
  const [editingId, setEditingId] = useState("")
  const [form, setForm] = useState<any>({ ...EMPTY_SIGNAL_SCHEMA, contractVersionId: initialContractVersionId })
  ...
  const reset = () => { setEditingId(""); setForm({ ...EMPTY_SIGNAL_SCHEMA, contractVersionId: initialContractVersionId }) }
  const save = async () => {
  ...
  return <Card>
    ...
    <CardContent className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(340px,1fr)]">
      ...
      <div className="space-y-4 rounded-md border p-4">
        ...
        <Button type="button" disabled={Boolean(busy) || !form.name.trim() || !form.templateName.trim() || !form.contractVersionId || (!editingId && !form.id.trim())} onClick={() => void save()}><Save className="mr-2 h-4 w-4" />{editingId ? "Schema speichern" : "Schema anlegen"}</Button>
      </div>
    </CardContent>
  </Card>
}
...
function Strategies({ data, busy, run }: any) {
  ...
  const positionSizingMode = cfg.sizing.positionSizingMode ?? "risk_percent"
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><div><CardTitle>Strategie-Editor</CardTitle><CardDescription>Alle Größen sind harte, validierte Verträge. Prozentwerte als Dezimalzahl.</CardDescription></div>{current?.status === "published" && <Button variant="outline" onClick={newVersion}>Neue Version aus v{current.version}</Button>}</div></CardHeader><CardContent className="space-y-6">
      ...
      <Section title="Sizing"><div className="grid gap-4 md:grid-cols-5"><SelectField label="Größenmodus" value={positionSizingMode} options={[{ value: "risk_percent", label: "Risiko bis Stop (%)" }, { value: "equity_percent_notional", label: "Positionswert (%)" }, { value: "equity_percent_margin", label: "Eingesetztes Kapital (%)" }]} onChange={v => patch("sizing", "positionSizingMode", v)} /><NumberField label={positionSizingMode === "equity_percent_margin" ? "Kapital / Trade (%)" : positionSizingMode === "equity_percent_notional" ? "Positionsgröße / Trade (%)" : "Basisrisiko / Trade (%)"} value={cfg.sizing.riskPerTradePercent} onChange={v => patch("sizing", "riskPerTradePercent", v)} /><NumberField label={positionSizingMode === "equity_percent_margin" ? "Max. adaptives Kapital (%)" : positionSizingMode === "equity_percent_notional" ? "Max. adaptive Positionsgröße (%)" : "Max. adaptives Risiko (%)"} value={cfg.sizing.maxAdaptiveRiskPercent ?? cfg.sizing.riskPerTradePercent} onChange={v => patch("sizing", "maxAdaptiveRiskPercent", v)} /><NumberField label="Absolute Notional-Obergrenze" value={cfg.sizing.maxPositionNotional} onChange={v => patch("sizing", "maxPositionNotional", v)} /><NumberField label="Max. Leverage" value={cfg.sizing.maxLeverage} onChange={v => patch("sizing", "maxLeverage", Number(v))} /></div>{positionSizingMode === "equity_percent_notional" && <p className="mt-3 text-xs text-muted-foreground">Der Positionswert entspricht dem gewählten Anteil der aktuellen Konto-Equity. Der Signal-Stop bestimmt den tatsächlichen Verlust, aber nicht die Positionsgröße.</p>}{positionSizingMode === "equity_percent_margin" && <p className="mt-3 text-xs text-muted-foreground">Das eingesetzte Kapital entspricht dem gewählten Anteil der aktuellen Konto-Equity. Der Positionswert ergibt sich aus eingesetztem Kapital × tatsächlich zulässigem Leverage und bleibt durch verfügbare Margin sowie die absolute Notional-Obergrenze begrenzt.</p>}{cfg.schemaVersion === 1 && <p className="mt-3 text-xs text-muted-foreground">Diese unveränderte Altversion verwendet nur das Basisrisiko. „Neue Version“ aktualisiert sie auf das adaptive Schema v2.</p>}</Section>
      ...
      <Section title="Harte Sicherheitslimits"><div className="grid gap-4 md:grid-cols-5"><NumberField label="Max. Positionen" value={cfg.safety.maxConcurrentPositions} onChange={v => patch("safety", "maxConcurrentPositions", Number(v))} /><Field label="Tagesverlust-Modus"><Select value={cfg.safety.maxDailyLossMode ?? "absolute"} onValueChange={v => patch("safety", "maxDailyLossMode", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="absolute">Fester Betrag</SelectItem><SelectItem value="equity_percent">% der Equity</SelectItem></SelectContent></Select></Field><NumberField label={cfg.safety.maxDailyLossMode === "equity_percent" ? "Max. Tagesverlust %" : "Max. Tagesverlust"} value={cfg.safety.maxDailyLoss} onChange={v => patch("safety", "maxDailyLoss", v)} /><NumberField label="Max. Slippage %" value={cfg.safety.maxSlippagePercent} onChange={v => patch("safety", "maxSlippagePercent", v)} /><NumberField label="Entry TTL (s)" value={cfg.safety.entryOrderTtlSeconds} onChange={v => patch("safety", "entryOrderTtlSeconds", Number(v))} /></div><div className="mt-3 flex items-center gap-2 text-sm"><CheckCircle2 className="h-4 w-4 text-primary" />Protective Stop ist zwingend und kann nicht deaktiviert werden.</div></Section>
...
function Accounts({ data, busy, run }: any) {
  const [form, setForm] = useState<any>({ name: "", exchange: "hyperliquid", mode: "testnet", initialBalance: "", privateKey: "", walletAddress: "", apiKey: "", apiSecret: "" })
  ...
  const save = async () => { await run("account", () => mutate(replaceId ? "/api/trading/accounts/credentials" : "/api/trading/accounts", replaceId ? { id: replaceId, credentials } : { name: form.name, exchange: form.exchange, mode: form.exchange === "paper" ? "paper" : form.mode, initialBalance: form.exchange === "paper" ? form.initialBalance : undefined, credentials }), replaceId ? "Zugangsdaten ersetzt und verifiziert." : "Konto angelegt und verifiziert."); clearSecrets(); setReplaceId("") }
  return <div className="space-y-5"><Card><CardHeader><CardTitle>{replaceId ? "Exchange-Keys ersetzen" : "Börsenkonto hinzufügen"}</CardTitle><CardDescription>Keys werden write-only gespeichert. Paper Trading wird nur nach ausdrücklicher Auswahl und mit einer selbst festgelegten Startbilanz angelegt.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 md:grid-cols-3"><Field label="Name"><Input value={form.name} disabled={Boolean(replaceId)} onChange={e => setForm({ ...form, name: e.target.value })} /></Field><SelectField label="Exchange" value={form.exchange} options={["hyperliquid", "bybit", "paper"]} disabled={Boolean(replaceId)} onChange={exchange => setForm({ ...form, exchange, mode: exchange === "paper" ? "paper" : "testnet", initialBalance: "" })} /><SelectField label="Modus" value={form.mode} options={form.exchange === "paper" ? ["paper"] : ["testnet", "live"]} disabled={Boolean(replaceId)} onChange={mode => setForm({ ...form, mode })} /></div>
    {!replaceId && form.exchange === "paper" && <NumberField label="Startbilanz (explizit festlegen)" value={form.initialBalance} onChange={initialBalance => setForm({ ...form, initialBalance })} />}
