...
export function MessagesTab() {
  ...
  const fetchMessages = async (signal?: AbortSignal) => {
    ...
    setError(null)
    try {
      const response = await apiFetch(`${API_BASE}/api/incoming-messages`, { signal })
  ...
  useSerializedPolling((signal) => fetchMessages(signal), 5_000)
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        ...
        <Button onClick={() => void fetchMessages()} variant="outline" size="icon" disabled={isLoading} aria-label="Nachrichten aktualisieren">
