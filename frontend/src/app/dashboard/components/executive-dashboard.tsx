...
function statusStyle(tone: "good" | "warning" | "bad" | "neutral"): string {
  if (tone === "good") return "border-emerald-500/20 bg-emerald-500/10 text-emerald-800 dark:text-emerald-300"
  if (tone === "warning") return "border-amber-500/20 bg-amber-500/10 text-amber-800 dark:text-amber-300"
  if (tone === "bad") return "border-red-500/20 bg-red-500/10 text-red-800 dark:text-red-300"
...
function ProgramActivity({ uptime, totalForwardedCount, processedSinceRestart, queue, parserEnabled, forwardingEnabled, forwardXmlToTarget, outboxCount }: Readonly<Pick<ExecutiveDashboardProps, "uptime" | "totalForwardedCount" | "processedSinceRestart" | "queue" | "parserEnabled" | "forwardingEnabled" | "forwardXmlToTarget"> & { outboxCount: number }>) {
  return <Card><CardHeader><CardTitle>Program Activity</CardTitle><CardDescription>Laufzeit-, Queue- und Routing-Kennzahlen.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><ActivityBox icon={<Clock3 className="h-5 w-5" />} label="Service Uptime" value={uptime} /><ActivityBox icon={<Route className="h-5 w-5" />} label="Weitergeleitet gesamt" value={number(totalForwardedCount)} /><ActivityBox icon={<Activity className="h-5 w-5" />} label="Seit Neustart" value={number(processedSinceRestart)} /><ActivityBox icon={<Layers3 className="h-5 w-5" />} label="Queue" value={`${queue.running} aktiv · ${queue.queued} wartend`} /><div className="flex flex-wrap gap-2 sm:col-span-2 xl:col-span-4"><Badge variant="outline" className={statusStyle(parserEnabled ? "good" : "neutral")}>KI-Parser {parserEnabled ? "aktiv" : "aus"}</Badge><Badge variant="outline" className={statusStyle(forwardingEnabled ? "good" : "neutral")}>Weiterleitung {forwardingEnabled ? "aktiv" : "aus"}</Badge><Badge variant="outline">{forwardXmlToTarget ? "XML-Zielmodus" : "Original-Zielmodus"}</Badge><Badge variant="outline">Outbox offen: {outboxCount}</Badge></div></CardContent></Card>
}
...
export function ExecutiveDashboard(props: ExecutiveDashboardProps) {
  ...
  return <div className="space-y-6 px-4 pb-10 lg:px-6">
    ...
    <ProgramActivity uptime={props.uptime} totalForwardedCount={props.totalForwardedCount} processedSinceRestart={props.processedSinceRestart} queue={props.queue} parserEnabled={props.parserEnabled} forwardingEnabled={props.forwardingEnabled} forwardXmlToTarget={props.forwardXmlToTarget} outboxCount={data.outbox?.length || 0} />
