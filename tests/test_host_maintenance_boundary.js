...
const evidence = JSON.parse(await read('docs/testing/host-maintenance-boundary-17abeb0f.json'));
...
assert.equal(evidence.archivalEvidence, 'docs/testing/host-maintenance-boundary-efbcb568.json');
for (const changedSource of ['docs/ui-next/inventory/operational-fields.json',
  ...
  assert.notEqual(archival.sourceSha256[changedSource], evidence.sourceSha256[changedSource],
    `${changedSource}: the archived PR-head hash must not be treated as the integration hash`);
}
...
assert.equal(evidence.baseRevision, audit.sourceRevision);
