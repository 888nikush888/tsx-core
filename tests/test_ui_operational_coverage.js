...
function validateCoverage(parameters, coverage) {
  ...
  assert.deepEqual([...classified.keys()].sort(), expected.sort(), 'Undocumented parameter in audited scope');
...
for (const group of inventory.groups) {
  for (const reference of [group.uiControl, group.backendValidator, group.contractTest].filter(Boolean)) await access(path.join(root, reference));
  if (group.mutationRoute) {
    const route = routes.find(item => item.route === group.mutationRoute);
