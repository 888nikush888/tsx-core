...
async function parserFailures() {
  ...
  let calls = 0; const provider = () => { calls++; throw new Error('provider must not run'); };
  ...
  await assert.rejects(runUiParserTest(accepted, () => { calls++; throw failure; }), error => error.cause === failure && /no automatic operator replay/.test(error.message));
