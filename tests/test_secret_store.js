...
try {
  ...
  await assert.rejects(async () => store.set({}), /At least one/);
  await assert.rejects(async () => store.set({ unknownSecret: 'value' }), /Unknown managed secret/);
  await assert.rejects(async () => store.set({ telegramApiHash: 'invalid' }), /32 hexadecimal/);
  await assert.rejects(async () => store.set({ backupEncryptionKey: 'not-base64' }), /canonical base64/);
  await store.set({
  ...
  await assert.rejects(
    async () => store.set({ backupEncryptionKey: Buffer.alloc(32, 8).toString('base64') }),
  ...
  await assert.rejects(async () => external.set({ telegramApiHash: 'c'.repeat(32) }), /externally managed/);
