...
try {
  ...
  bridge = new McpControlBridge(control, { record: () => Promise.resolve() }, () => undefined, 50, authority);
  await bridge.start();
  await delay(130);
  ...
  bridge = new McpControlBridge(control, { record: () => Promise.resolve() }, () => undefined, 50, nextStartup);
  await bridge.start();
  await delay(130);
