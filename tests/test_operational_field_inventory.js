...
const strategyAccessEntrySlice = firstSlice.groups.find(group => group.id === 'strategy-access-entry');
assert.equal(strategyAccessEntrySlice.class, 'versioned-draft');
assert.deepEqual(strategyAccessEntrySlice.paths, [
  'strategy.allowedSignalSchemas', 'strategy.allowedSymbols', 'strategy.allowedSides',
  'strategy.entry.orderType', 'strategy.entry.rangePrice', 'strategy.entry.postOnly', 'strategy.entry.timeoutSeconds',
]);
assert.deepEqual(strategyAccessEntrySlice.contractTests, [
  'frontend/tests/workflow-resource-editor.test.tsx',
  'tests/test_web_server.js', 'tests/test_workflow_builder.js',
]);
for (const testFile of strategyAccessEntrySlice.contractTests) await access(path.join(root, testFile));
...
assert.deepEqual(adaptiveGroup.paths, [
  'resource.adaptive_risk.enabled',
...
assert.equal(enabledField.evidenceStatus, 'first-slice-static', 'Disabled-policy engine effect has a no-order contract test');
assert.equal(enabledField.firstSliceGroup, 'adaptive-risk-policy');
