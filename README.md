...
Ein zweiter, read-only Self-hosted Runner mit den Labels `self-hosted, production-observer` prüft vor Live-GO einen 24-Stunden-Prometheus-Rückblick (`npm run ops:release-observation`); der 30-Tage-Report (`npm run ops:soak`) bleibt für das fortlaufende SLO erhalten und ist keine Wartepflicht. Bis solche Runner eingerichtet sind, sind Staging-, Synthetic- und Beobachtungs-Workflows ausschließlich manuell auslösbar. Für echten Produktionsbetrieb muss ein externer Scheduler den 30-Tage-Observer täglich und den Synthetic-Monitor alle 15 Minuten starten sowie ausgebliebene Läufe alarmieren. Die Releasebeobachtung fordert pro aktiver Serie mindestens 5732 von nominell 5760 Scrapes bei 15-Sekunden-Takt (99,5 % Stichprobenabdeckung, keine lückenlose Zeitmessung), 99,5 % Verfügbarkeit und bestätigte Zustellung, P95 unter 60 Sekunden, mindestens 100 Zustellversuche und 100 Paper-/Testnet-Trade-Intents, keine unbekannte Zustellung oder Order und keine ungeschützte Position, gesunde Backups/Retention/Disk sowie begrenzte Queue und RAM. Echte Provider-Lifecycles, Restore, TLS, gewählter Live-Host und Freigaben bleiben eigenständige Nachweise. `PROMETHEUS_URL` muss HTTPS verwenden (Loopback ausgenommen), das optionale Bearer-Token kommt aus `PROMETHEUS_TOKEN[_FILE]`.
