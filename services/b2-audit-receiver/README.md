...
3. The package is bound into Quality OS test, dependency, license, SBOM, and `BUILD_INPUT_POLICY` gates in this candidate. Its two logic modules also have local 90% line, 80% branch and 90% function coverage floors; all three production files are in the Sonar static scope, while the bootstrap `server.mjs` is excluded from coverage. Codacy receives the same logic-module report. The final merged branch still needs exact-head cloud verification of these gates and container/image policy. A standalone package without those checks is not sufficient.
