...
It accepts the relay's Bearer token and exact `X-Alert-Source: tsx-core` header at `POST /alerts`. It bounds the body to 1 MiB, validates zero through 100 alerts, and rejects positive `truncatedAlerts` rather than acknowledging an incomplete group. Each complete input produces one Telegram `sendDocument` call. The attached JSON preserves every received alert's `alertname`, `severity`, optional `service`, and optional `correlation_id` values exactly, including long, empty, escaped, and Unicode strings. Annotations, other labels, and the raw body are not forwarded to Telegram, stored in D1, or logged. `GET /healthz` establishes process response only; host-down monitoring is separate.

A D1 row with a unique HMAC delivery key is reserved before Telegram. The same key is printed as a receipt in the Telegram caption so an operator can match an uncertain D1 row to a document. A duplicate receives 202 only after previously confirmed Telegram delivery and D1 completion. Pending or uncertain deliveries return 503 without another Telegram call. A verified Telegram 429 with `ok:false` and bounded `retry_after` is the sole automatic retry case. Successful rows are deduplicated for 20 minutes, allowing Alertmanager's current 30-minute reminders. A network uncertainty or post-send D1 failure stays blocked for manual reconciliation; Telegram has no idempotency key. A 202 does not mean a human viewed the document.
