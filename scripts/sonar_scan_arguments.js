...
export function sonarScanArguments(environment) {
  ...
  const args = [`-Dsonar.scm.revision=${environment.SONAR_EXPECTED_REVISION}`];
  if (pullRequest) args.push(
    `-Dsonar.pullrequest.key=${environment.SONAR_PULL_REQUEST}`,
    `-Dsonar.pullrequest.branch=${environment.SONAR_PULL_REQUEST_BRANCH}`,
    `-Dsonar.pullrequest.base=${environment.SONAR_PULL_REQUEST_BASE}`
  );
  else if (environment.SONAR_BRANCH) args.push(`-Dsonar.branch.name=${environment.SONAR_BRANCH}`);
  return args.join(' ');
}
