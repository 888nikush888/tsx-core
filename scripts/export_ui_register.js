...
const capabilityRows = capabilities.map(entry => `| ${[entry.route, entry.role, entry.href, `${entry.scope}. ${entry.effect}`,
  entry.boundary ?? 'Operator surface', entry.contractTests.join(', ')].map(cell).join(' | ')} |`).join('\n');
const parameterRows = [...new Set(parameters.map(entry => entry.path.split('.')[0]))]
  .map(family => `| ${family} | ${parameters.filter(entry => entry.path.startsWith(`${family}.`)).length} |`).join('\n');
const markdown = [
