...
function validRationale(value) {
  return typeof value === 'string' && value.trim() === value && value.length >= 80 && value.length <= 4000
    && !/\p{Cc}/u.test(value); // match any control characters (Unicode Cc)
}
...
function authorizeRunner(environment, mode) {
  ...
  requireCondition(environment.GITHUB_REF_TYPE === 'branch' && /^refs\/heads\/[^[\s\p{Cc}]+$/u.test(environment.GITHUB_REF ?? '')
