...
// Refs are data: API requests encode query parameters and scanner arguments
// use fixed environment placeholders. Reject interpolation, not shell punctuation.
function branchScope(environment) {
  if (environment.SONAR_BRANCH && /[\s\x7f]|\$\{/u.test(environment.SONAR_BRANCH)) {
...
function validatePullRequestSelection(key, branch, base, selectedBranch) {
  if (!/^[1-9]\d*$/u.test(key) || !branch || !base || selectedBranch) {
    throw new Error('SonarCloud pull request scope requires a numeric key, source and target; it cannot also select a branch.');
  }
}

function pullRequestScope(environment, key) {
  const branch = environment.SONAR_PULL_REQUEST_BRANCH?.trim();
  const base = environment.SONAR_PULL_REQUEST_BASE?.trim();
  validatePullRequestSelection(key, branch, base, environment.SONAR_BRANCH?.trim());
...
export function sonarScope(environment) {
  ...
  if (!key) return branchScope(environment);
  return pullRequestScope(environment, key);
}
...
function validateBranchIdentity(identity, configuration) {
  if (identity.revision !== configuration.expectedRevision || identity.analysisBranch !== configuration.branch) {
    throw new Error('SonarCloud compute task does not prove the expected branch revision and scope.');
  }
  if ([identity.pullRequest, identity.branch, identity.base].some(value => value !== undefined)) {
    throw new Error('SonarCloud compute task does not prove the expected branch revision and scope.');
  }
}
...
export function validateBranchTask(task, configuration) {
  ...
  if (!analysisId) throw new Error('SonarCloud compute task does not prove the expected branch revision and scope.');
  validateBranchIdentity(identity, configuration);
  if (pullRequest !== undefined) throw new Error('SonarCloud compute task does not prove the expected branch revision and scope.');
}

function validateLongBranch(branch, configuration, analysis) {
  if (branch.type !== 'LONG' || branch.isMain !== false || branch.commit?.sha !== configuration.expectedRevision) {
    throw new Error('SonarCloud branch does not prove a full long-lived analysis at the expected revision.');
  }
  validateLongBranchDate(branch.analysisDate, analysis.date);
}

function validateLongBranchDate(branchDate, analysisDate) {
  if (!Number.isFinite(Date.parse(branchDate)) || Date.parse(branchDate) !== Date.parse(analysisDate)) {
    throw new Error('SonarCloud branch does not prove a full long-lived analysis at the expected revision.');
  }
}
...
export async function readLongBranchIdentity(configuration, analysis, options) {
  ...
  validateLongBranch(branch, configuration, analysis);
...
export function validatePullRequestTask(task, configuration) {
  ...
  validatePullRequestChecks(checks);
}

function validatePullRequestChecks(checks) {
...
function analysisMatchesRevision(analysis, expectedRevision) {
  return analysis.commit?.sha === expectedRevision && Number.isFinite(Date.parse(analysis.analysisDate));
}

function validatePullRequestAnalysis(request, scope, expectedRevision) {
  if (request.branch !== scope.branch || request.base !== scope.base
    || !analysisMatchesRevision(request, expectedRevision)) {
...
export async function readPullRequestAnalysis(configuration, computeTask, options) {
  ...
  validatePullRequestAnalysis(request, scope, configuration.expectedRevision);
  return { key: computeTask.analysisId, date: request.analysisDate, revision: request.commit.sha };
}

function periodsValue(periods) {
  if (!Array.isArray(periods) || periods.length !== 1 || periods[0]?.index !== 1) return null;
  return periods[0].value;
}

function periodValue(period) {
  if (!period || Array.isArray(period) || (period.index !== undefined && period.index !== 1)) return null;
  return period.value;
}

function rawMetricValue(entry, shape) {
  if (shape === 'periods') return periodsValue(entry.periods);
  if (shape === 'period') return periodValue(entry.period);
  return entry.value;
}
...
function metricValue(measures, metric) {
  ...
  const raw = rawMetricValue(entry, shapes[0]);
  if (typeof raw !== 'string' || !/^\d+(?:\.\d+)?$/u.test(raw)) return null;
  return Number(raw);
}

function reviewMeasures(response, configuration) {
  const component = response.component ?? {};
  const measures = component.measures;
  if (component.key !== configuration.projectKey || component.pullRequest !== configuration.pullRequest.key
...
function validateHotspotCount(count, reviewedPercent) {
...
function validateReviewPercent(hasReviewMeasure, reviewedPercent) {
  if (hasReviewMeasure && (reviewedPercent === null || !Number.isFinite(reviewedPercent) || reviewedPercent > 100)) {
    throw new Error('SonarCloud pull request hotspots are unreviewed or their review status is unproven.');
  }
}
...
export async function readPullRequestHotspotReview(configuration, options) {
  ...
  const measures = reviewMeasures(response, configuration);
  ...
  validateHotspotCount(count, reviewedPercent);
  validateReviewPercent(hasReviewMeasure, reviewedPercent);
