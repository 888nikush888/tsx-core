...
const [backendLock, frontendLock] = await Promise.all([
...
const pythonDirectRequirements = await readFile(path.join(root, 'exchange_executor', 'requirements.in'), 'utf8')
  .catch(error => error?.code === 'ENOENT' ? null : Promise.reject(error));
const pythonLockedRequirements = pythonDirectRequirements === null
  ? null
...
const results = [
  ...
  ...(pythonDirectRequirements === null || pythonLockedRequirements === null
    ? []
...
if (violations.length > 0) {
  ...
  console.log(`License policy passed (${inventory.length} locked dependency artifacts).`);
}
