...
import { classifyAiError, DEFAULT_AI_LIMITS, loadSignalPromptTemplate, parseSignalToXml, type AiLimits, type ParseSignalOptions } from './signal_parser.js';
...
export async function uiParserMetadata(config: { xmlParsing?: unknown }, queue: unknown) {
  const workflow = await getActiveWorkflow(); const usageDay = new Date().toISOString().slice(0, 10);
  const xmlParsing = (config.xmlParsing ?? {}) as Record<string, unknown>;
  return { version: 1, observedAt: Date.now(), usageDay, usage: await getAiUsage(usageDay), queue,
    limits: { ...DEFAULT_AI_LIMITS, ...(xmlParsing.aiLimits as Partial<AiLimits> | undefined) }, primaryModel: xmlParsing.primaryModel ?? null,
    fallbackModel: xmlParsing.fallbackModel ?? null, externalDataPolicyAccepted: xmlParsing.externalDataPolicyAccepted === true,
...
function testLimits(xml: Record<string, unknown>) {
  const limits = { ...DEFAULT_AI_LIMITS, ...(xml.aiLimits as Partial<AiLimits> | undefined) };
  for (const [key, [minimum, maximum]] of Object.entries(AI_LIMIT_RANGES)) {
    const value = limits[key as keyof AiLimits];
    if (!Number.isSafeInteger(value) || value < minimum || value > maximum) throw new Error(`Invalid configured AI limit: ${key}.`);
  }
  return limits;
}
...
async function selectedTestParser(pathId: string) {
  ...
  const resources = selected?.effectiveConfiguration.resources as Record<string, unknown> | undefined;
  const parser = (resources?.parser ?? {}) as { templateName?: string; prompt?: string; primaryModel?: string; fallbackModel?: string; timeoutMs?: number };
...
async function selectedTestSchema(context: Awaited<ReturnType<typeof selectedTestParser>>) {
  ...
  const schema = selected ? (await listTradingSignalSchemas()).find(item => item.id === (resources?.schema as { schemaId?: unknown } | undefined)?.schemaId) : await getTradingSignalSchemaForTemplate(templateName);
  const contractId = selected ? (resources?.contract as { contractVersionId?: string } | undefined)?.contractVersionId : schema?.contractVersionId;
...
export async function prepareUiParserTest(config: { xmlParsing?: unknown }, input: { pathId?: unknown; sourceText?: unknown }) {
  const pathId = identifier(input.pathId); const xml = structuredClone((config.xmlParsing ?? {}) as Record<string, unknown>);
  ...
  const models = { primaryModel: parser.primaryModel || (xml.primaryModel as string | undefined), fallbackModel: parser.fallbackModel || (xml.fallbackModel as string | undefined) };
...
function boundedXml(xml: string): string {
  ...
  if (low && xml[low - 1] !== undefined && /[\uD800-\uDBFF]/u.test(xml[low - 1] as string)) low--;
