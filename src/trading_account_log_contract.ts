...
function token(value: unknown, nullable = false, maximum = 4096): string | null {
  ...
  if (typeof value !== 'string' || !value || value.length > maximum || /[\u0000-\u001F]/.test(value)) throw new Error('Invalid account-log token.');
  return value;
}
...
function record(value: unknown, namespace: string): AccountLogRecord {
  ...
  for (const [field, item] of Object.entries(row)) {
    ...
    if (item !== null && (typeof item !== 'string' || item.length > 256 || /[\u0000-\u001F]/.test(item))) throw new Error('Invalid account-log economics.');
