...
import { uiTlsStatus } from './ui_tls_status.js';
...
export async function uiDeployment(listener: { address: string | undefined; port: number | undefined }, activeDashboardCertificate?: Buffer) {
  const [memory, cpu, tls] = await Promise.all([
    kernelLimit('/sys/fs/cgroup/memory.max', /^(max|\d{1,20})$/),
    kernelLimit('/sys/fs/cgroup/cpu.max', /^(max|\d{1,20}) \d{1,20}$/),
    uiTlsStatus(activeDashboardCertificate),
  ]);
