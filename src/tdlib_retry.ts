...
export async function invokeWithFloodWaitRetry(
  ...
  if (!Number.isSafeInteger(maxFloodWaitSeconds) || maxFloodWaitSeconds < 0 || maxFloodWaitSeconds > 3_600) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      const match = /FLOOD_WAIT_(\d+)/.exec(String(error?.message || error));
      if (!match) throw error;
      const waitSeconds = Number(match[1]);
      if (!Number.isSafeInteger(waitSeconds) || waitSeconds > maxFloodWaitSeconds) {
        throw new Error(
          `Telegram FLOOD_WAIT_${match[1]} exceeds the configured ${maxFloodWaitSeconds}s safety limit.`,
          { cause: error }
        );
      }
