...
function accountFingerprintOf(account: TradingAccount): string {
  const fingerprint = account.externalAccountId;
  if (!fingerprint) throw new KrakenCashlegError('account_binding_unproven');
  return fingerprint;
}

function executionIdsOf(related: KrakenCashlegOccurrence[]): string[] {
...
async function soleFeeEvent(accountId: string, fingerprint: string, executions: string[]): Promise<{ id: string; execution: string }> {
  const events = await getDatabase().all<Array<{ id: string; execution: string }>>(`SELECT event.id,fills.exchange_fill_id AS execution
    ...
    WHERE event.account_id=? AND event.account_fingerprint=? AND event.kind='fee' AND event.basis='fill'
      AND fills.exchange_fill_id IN (${executions.map(() => '?').join(',')}) LIMIT 2`, [accountId, fingerprint, ...executions]);
...
export async function projectKrakenCashleg(account: TradingAccount, row: AccountLogRecord): Promise<void> {
  ...
  const executions = executionIdsOf(related);
  const event = await soleFeeEvent(account.id, fingerprint, executions);
