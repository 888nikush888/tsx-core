...
interface FlattenPrevious {
  client_order_id: string; status: string; quantity: string; filled_quantity: string; updated_at: number;
}

function assertNoConflictingFlatten(previous: FlattenPrevious | undefined): void {
...
function assertFlattenTerminalEvidence(previous: FlattenPrevious | undefined): void {
...
function assertFlattenCooldownElapsed(previous: FlattenPrevious | undefined): void {
...
async function retireChangedFlattenPreparation(intentId: string, previous: FlattenPrevious | undefined, quantity: string): Promise<void> {
  if (previous?.status === 'created' && compareDecimal(previous.quantity, quantity) !== 0) {
    if (!await retireUndispatchedExit(intentId, previous.client_order_id)) {
...
export async function prepareEmergencyReduction(account: TradingAccount, intent: TradingIntent, quantity: string): Promise<PlannedOrder> {
  ...
  assertNoConflictingFlatten(previous);
  assertFlattenTerminalEvidence(previous);
  assertFlattenCooldownElapsed(previous);
  await retireChangedFlattenPreparation(intent.id, previous, quantity);
