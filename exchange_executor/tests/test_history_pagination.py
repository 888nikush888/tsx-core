...
class PaginationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_more_than_500_orders_and_1000_fills_with_empty_and_overlapping_pages(self):
        ...
        missing = object()
        orders_state = next((row for row in states if row["source"] == "orders"), missing)
        self.assertIsNot(orders_state, missing, "Expected an orders pagination state.")
        self.assertEqual(orders_state["completeness"], "unknown", "Shorter unfilled-order retention must stay explicit.")
