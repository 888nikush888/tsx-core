...
class AcceptanceJournalTests(unittest.TestCase):
    ...
    def test_real_fsync_failure_prevents_grant_and_poisons_owner(self):
        ...
        with (
            patch('acceptance_journal.os.fsync', side_effect=disk_error),
            self.assertRaisesRegex(JournalRefused, 'durability') as error,
        ):
            guard.mark_dispatching("own-1")
    ...
    def test_expired_or_stale_final_fsync_cannot_return_a_send_permit(self):
        ...
        with (
            patch('acceptance_journal.os.fsync', side_effect=delayed_sync),
            self.assertRaisesRegex(JournalRefused, 'time budget'),
        ):
            guard.mark_dispatching("own-1")
        journal.close()
