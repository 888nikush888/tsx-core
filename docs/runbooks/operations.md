...
4. Im ausgelieferten Repository sind diese Self-hosted Workflows bis zur Runner-Einrichtung nur manuell auslösbar. Vor Live-GO `Release Observation Evidence` auf dem Release-SHA ausführen. Für den laufenden Produktionsbetrieb einen extern überwachten Scheduler konfigurieren, der `Staging Release Gate` nach jedem erfolgreichen Main-Build, `Staging Synthetic Monitor` alle 15 Minuten und `Production 30-Day Evidence` täglich startet. Das externe Incident-System muss ausgebliebene Starts und Runner-Offline-Zustände alarmieren, weil ein nicht gestarteter Workflow keinen Anwendungsalarm erzeugt.
5. Vor dem Release die JSON-Artefakte unter `reports/staging/` und `reports/soak/` dem Release-Record zuordnen. Sie müssen zum Release-SHA, zum tatsächlichen Monitoring-/Staging-Deployment und zum freizugebenden Slice/Konto passen. Die Releasebeobachtung fragt 24 Stunden Prometheus-Daten zurück und fordert pro aktiver Serie mindestens 5732 von nominell 5760 Scrapes bei 15-Sekunden-Takt (99,5 % Stichprobenabdeckung, keine lückenlose Zeitmessung), 100 Zustellversuche und 100 kontrollierte Paper-/autorisierte Testnet-Intents sowie alle Gesundheits- und Null-Unknown-Prüfungen. Die Intent-Metrik stammt aus der Zahl persistierter DB-Zeilen; deshalb jede der 100 eindeutigen Journal-IDs mit Zeitfenster, Staging-Source-SHA, Konto und Paper-/Testnet-Modus unabhängig gegenprüfen. Keine künstlichen Erfolgseinträge, keine 100 Live-Orders. Zusätzlich echte Provider-Lifecycle-, Restore-, TLS- und Incidentproben belegen. `Production 30-Day Evidence` bleibt ein nachlaufender SLO-Report und ist keine kalendarische Live-GO-Sperre.
