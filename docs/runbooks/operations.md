...
5. Vor dem Release die JSON-Artefakte unter `reports/staging/` und `reports/soak/` dem Release-Record zuordnen. Sie müssen zum Release-SHA, zum tatsächlichen Monitoring-/Staging-Deployment und zum freizugebenden Slice/Konto passen. Die Releasebeobachtung fordert 24 Stunden mit mindestens 5732 Scrapes bei 15-Sekunden-Takt, 100 Zustellversuchen und 100 kontrollierten Paper-/autorisierten Testnet-Intents sowie alle Gesundheits- und Null-Unknown-Prüfungen. Die Intent-Metrik stammt aus der Zahl persistierter DB-Zeilen; deshalb 100 eindeutige Journal-IDs, Konto/Modus, Zeitfenster und den Release-SHA unabhängig gegenprüfen. Keine künstlichen Erfolgseinträge, keine 100 Live-Orders. Zusätzlich echte Provider-Lifecycle-, Restore-, TLS- und Incidentproben belegen. `Production 30-Day Evidence` bleibt ein nachlaufender SLO-Report und ist keine kalendarische Live-GO-Sperre.
