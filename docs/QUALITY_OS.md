...
| MCP-Agentensteuerung | gültiger gehashter Agenten-Bearer, aktive Sitzung, aktuelles dauerhaftes Minimalrecht, valides Tool-Schema | begrenztes Read-Ergebnis oder auditierte Kontrollantwort; bestätigte Löschung entfernt den Agenten aus dem aktiven Inventar | keine Exchange-Secrets/Adapter im MCP-Prozess; Recht pro Aufruf erneut prüfen; Löschung widerruft Token/Sitzungen/Requests sofort und bewahrt anonymisierte Audit-Referenzen; Write nur via persistente Bridge + Vorab-Audit + `TradingWebControl`; Wartungsmarker vor DB-Ersatz | Agenten-/Sitzungs-/Aktions-/Request-/Event-Zustand; mögliche Trading-Aktion | Auth/Recht/Schema/Audit/Bridge-Timeout = fail closed; Event-Fehler retryfähig; keine direkte DB-Order | Agenten-Aktionen, Audit, Sessions, Event-Delivery, Latenz | `test_mcp_control_plane.js`, `test_mcp_server.js`, `test_web_server.js`, `frontend/tests/mcp-agents-tab.test.tsx` | Security + Trading Domain + SRE |
