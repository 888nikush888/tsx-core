...
| Business-/Systemziel | Input Contract | Output Contract | Invarianten | Side Effects | Failure Policy | Observability | Test Evidence | Owner |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Standalone-Webzugang ohne Token-Dialog | Host-Loopback, Token-Modus, `dashboardLocalTrust=true`, Same-Origin und Dashboard-Header | Beim Erststart einmalig angezeigter Admin-Key; danach höchstens zwölf Stunden gültiger In-Memory-Session-Token | Dauerhafter Admin-Key wird nie erneut offengelegt; Enterprise/OIDC deaktiviert den Pfad; lokale Prozesse des dedizierten Single-User-Hosts liegen in der Trust Boundary | Verwalteten Admin-Key einmalig erzeugen oder kurzlebige lokale Sitzung ausstellen | Abweichende Origin/Betriebsart/Secret-Quelle wird abgewiesen; beschädigte Managed-Dateien starten nur eine lokale Repair-Plane | Security-/Audit-Log, Request-ID; klar markierter Recovery-Log bei fehlender Audit-Kette | `test_dashboard_auth.js`, `test_web_server.js`, `frontend/e2e/dashboard.spec.ts`, Docker-Startup-Smoke | Security Owner |
