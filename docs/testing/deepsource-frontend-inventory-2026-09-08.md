...
This is an in-progress ledger, not a claim that the frontend scan is clean. At implementation commit a5b7b86, 259 occurrences have a local fix awaiting rescan, 10 have a concrete proposed false-positive rationale, and 578 remain open for review. No platform status, rule or quality threshold was changed. Hash binding identifies the reviewed text; it does not make a pending occurrence reviewed.
