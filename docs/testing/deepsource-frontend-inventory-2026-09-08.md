...
This is an in-progress ledger, not a claim that the frontend scan is clean. At implementation commit18a3c29,89 occurrences have a local fix awaiting rescan, 8 have a concrete proposed false-positive rationale, and 750 remain open for review. No platform status, rule or quality threshold was changed. Hash binding identifies the reviewed text; it does not make a pending occurrence reviewed.
