...
Stand: 2026-09-07. Diese Anleitung beschreibt die Nachweisgrenzen; sie erteilt keine neue Freigabe für Testnet, Livehandel oder Deployment.
...
**Aktueller Implementierungsstatus:** Der unabhängige Reviewanker `exchange_executor/ccxt_implementation_reviews.py` enthält einen geprüften Hyperliquid-Receipt. Bybit und Kraken Futures bleiben ohne Implementierungspin in Quarantäne. Die tatsächlichen aktuellen Source-, SDK- und Receiptbytes müssen bei jedem Root-/Runtimevergleich übereinstimmen; ein alter Pin allein genügt nicht. Die Reviewakten liegen unter `exchange_executor/certifications/reviews/`. `providerAcceptanceVerified` bleibt false: Implementierungsreview und Offline-Tests ersetzen keine echte Providerabnahme.

## Implementierungs-Receipt v2
...
Das Werkzeug ist **kein Testrunner und kein Pin-Generator**. Vergleichserfolg bedeutet nur, dass zuvor unabhängig genehmigte Receiptbytes zur aktuellen Source und zum geprüften Python-/SDK-Stand passen. `performedGateExecution` und `providerAcceptanceVerified` bleiben false. Bei Source- oder Receiptabweichungen liefert die produktive CLI No-Go. Der separate synthetische Testvergleich erzeugt keine Runtime-/Implementierungsfreigabe.

Der Containerjob in `quality.yml` hängt vom vollständigen `verify`-Job ab, lädt die vollständige Git-Historie und führt denselben Root-Gate vor dem Imagebau mit gesperrten Dependencyversionen aus. Es gibt keinen Fehler- oder Profilskip. Das Ergebnis wird als Containerprüfartefakt aufbewahrt. Der Nachweis für einen konkreten Commit ist sein [Quality-OS-Lauf](https://github.com/888nikush888/tsx-core/actions/workflows/quality.yml) samt Artefakten; ein früherer grüner Lauf bestätigt keine späteren Änderungen. Ein direkter, außerhalb dieses Workflows gestarteter `docker build exchange_executor` ersetzt diesen Root-Nachweis nicht.

Der vollständige positive CLI-Test verwendet ausschließlich ein eigenes temporäres Git-Repository, Testpins und eine bytegeprüfte, nicht hartverlinkte Kopie des installierten SDK. Negative Fälle zeigen zusätzlich: Node-Drift kann trotz gültigem Runtime-Receipt den Root-Gate sperren; ein falscher SDK-Hash sperrt beide Ebenen. Diese Testpins werden nicht in den wirklichen Reviewanker übernommen. SDK-Installationen mit Hardlinks werden unabhängig vom Versionsstring abgewiesen. Ein grüner temporärer Test ist kein Nachweis einer bereits geeigneten Entwicklungs- oder Containerinstallation; dafür wird die tatsächlich verwendete Installation separat geprüft.

Der zusätzliche argumentlose `verify_implementation_runtime.py` prüft deshalb auch den **fertigen** Executor: Python-/CCXT-Version, nichtleeres vollständiges Profilinventar und für jedes Profil die tatsächliche `certification_result`-Prüfung. CI führt ihn unter dem zuvor geprüften Imagebenutzer 65532, mit `--network none --read-only` und ohne Quellmounts aus. Mindestens ein gültig gepinntes Profil ist erforderlich; Profile mit ausdrücklich ausstehendem Implementierungsreview bleiben als Quarantäne ausgewiesen. Andere ungültige Receiptzustände brechen den Vergleich ab. Root-Sourceprüfung und Providerabnahme werden dadurch ausdrücklich nicht ersetzt. Reguläre pip-Extraktion kann unabhängige Dateien erzeugen; weder `--require-hashes` noch ein Versionsimport ist ein Beleg für den finalen Dateizustand.
