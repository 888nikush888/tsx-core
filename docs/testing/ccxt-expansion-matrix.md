...
Der historische Runtime-Reviewanker enthält den revisions- und quellgenau gebundenen Hyperliquid-V2-Receipt im verengten Master-Key-only-/USDC-Perp-Scope. Der additive [Agent-Wallet-Kandidatenreview](../../docs/testing/hyperliquid-agent-wallet-candidate-review-2026-09-25.md) bindet einen separaten Testnet-Agentpfad mit Grant-, Transport- und Credential-Generation-Negativfällen; er erteilt keine Live- oder Providerfreigabe. Bybit und Kraken Futures bleiben trotz vorhandener Profildeklarationen quarantined: Bybit wegen unbewiesener accountweiter Option-/Pre-upgrade-/delisteter Fillfinalität, Kraken wegen des unbewiesenen geschützten IOC+Stop-Vertrags und weiterer Quellenlücken. Alte Sieben-Bool-Dateien erteilen keine Freigabe. Der Server wurde nicht geändert; Providerabnahme bleibt getrennt. Runtime-/Build-/Providergrenzen stehen im [Bridge-Review](../../plans/009-CERTIFICATION-BRIDGE-REVIEW.md).
