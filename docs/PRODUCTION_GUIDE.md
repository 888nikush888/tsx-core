...
Beim ersten Browseraufruf ist noch keine lokale Sitzung zulässig. Eine direkte Loopback-Verbindung darf den sichtbaren Erststart ausführen. Da Docker-Portweiterleitung im Container nicht als Loopback ankommt, verlangt das Standard-Compose dort zusätzlich `DASHBOARD_BOOTSTRAP_PROOF`: einen frischen, mindestens 32 Zeichen langen Zufallswert, den der Operator beim Start übergibt und einmalig in der UI eingibt. Nach erfolgreicher Verwendung entfernt der Prozess den Nachweis und ein bereits konfigurierter Admin-Bearer sperrt jede Wiederverwendung. Origin und `X-Requested-With` bleiben zusätzliche Request-Integritätsprüfungen, sind aber kein Herkunftsnachweis. Der Browser hält den danach erzeugten dauerhaften Admin-Bearer nur im Session Storage. `/api/local-session` ist kein automatischer Login mehr: Der Endpunkt verlangt zusätzlich den gültigen dauerhaften Admin-Bearer und gibt dann ausschließlich einen separaten, höchstens zwölf Stunden gültigen In-Memory-Session-Token zurück. Dieser Token wird sofort ungültig, sobald der Admin-Bearer rotiert oder Local Trust deaktiviert wird. Remote-, Shared-Host-, Tailscale- und Enterprise-Zugriffe erhalten keine lokale Sitzung. Telegram API Hash und OpenRouter-Key werden im Dashboard write-only gesetzt: Status und Quelle sind lesbar, der Wert selbst nie. Das Standard-Compose reicht aus der Shell nur den ausdrücklich benannten Bootstrap-Nachweis weiter und liest keine übrigen lokalen Secrets als Containerumgebung ein.

Ist eine verwaltete Konfiguration, Runtime-Einstellung oder Secret-Datei beschädigt, startet nur die Recovery-Control-Plane: Routing, Scheduler und Datenbankzugriffe bleiben aus. Direkter Hostbetrieb nutzt die Loopback-Grenze; Compose verlangt für einen neuen Recovery-Bootstrap ebenfalls einen frisch gesetzten `DASHBOARD_BOOTSTRAP_PROOF` und einen neu erzeugten Container. Der Pfad erlaubt nur Reparaturen an Konfiguration, Runtime-Einstellungen und verwalteten Secrets plus Neustart und schreibt explizite kritische Recovery-Logs, weil die normale Audit-Kette in diesem Zustand nicht verfügbar sein kann. Beim Recovery-Neustart endet der Prozess und damit jede In-Memory-Session.
...
| Releasebeobachtung und fortlaufendes SLO | **NICHT VERIFIZIERT** ohne tatsächliche Staging-Messwerte | Vor GO `npm run ops:release-observation`: 24-Stunden-Prometheus-Rückblick, pro aktiver Serie mindestens 5732 von nominell 5760 Scrapes bei 15-Sekunden-Takt (99,5 % Stichprobenabdeckung, keine lückenlose Zeitmessung), 99,5 % Verfügbarkeit/Erfolg, mindestens 100 Zustellversuche und kontrollierte Paper-/Testnet-Intents, keine `unknown`/ungeschützten Positionen, P95 und Ressourcenlimits eingehalten. `npm run ops:soak` berichtet erst danach das laufende 30-Tage-SLO, ohne kalendarische GO-Sperre. |
